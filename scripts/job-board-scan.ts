...
import {
  assertBbBrowserAvailable,
  bbFetch,
} from "../packages/browser/src/bb-shim.ts";
import { BrowserController } from "../packages/browser/src/browser-controller.ts";
import { searchBuiltIn } from "../packages/browser/src/sites/builtin/index.ts";
import { searchIndeed } from "../packages/browser/src/sites/indeed/index.ts";
...
async function runSiteAdapter(options: Options, page: number): Promise<AdapterResult> {
  // Migrated from spawning `bb-browser site <name>/jobs ...` to calling
  // the in-process @auto-job/browser site adapters. The adapters return
  // the same shape (source/url/query/location/page/count/totalParsed/jobs)
  // as the old envelope.data, with typed jobs.
  const controller = await BrowserController.ensure();
  const tab = await controller.openTab(
    options.source === "builtin" ? "https://builtin.com/jobs" : "https://www.indeed.com/jobs",
  );
  try {
    if (options.source === "builtin") {
      ...
      const r = await searchBuiltIn(tab, { query, limit: adapterLimit(options), path, page });
      return r as unknown as AdapterResult;
    } else {
      const explicitUrl = options.url ? buildIndeedPageUrl(options.url, page) : null;
      ...
      const opts: { query: string; location: string; limit: number; page: number; url?: string } = {
        query,
        location,
        limit: adapterLimit(options),
        page,
      };
      if (explicitUrl) opts.url = explicitUrl;
      const r = await searchIndeed(tab, opts);
      return r as unknown as AdapterResult;
    }
  } finally {
...
// assertBbBrowserAvailable now imported from @auto-job/browser/bb-shim.
...
async function captureDetailText(source: Source, url: string, fallbackText: string): Promise<string> {
  const { stdout } = await bbFetch(url);
