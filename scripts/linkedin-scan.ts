...
// bb-browser legacy types (BbJsonEnvelope, BbOpenData, BbTabListData, BbEvalData)
// removed in the own-browser migration. BbTabInfo now sourced from
// `@auto-job/browser/bb-shim` (imported above) — same shape consumers care
// about: { tabId, url, title }.
...
// assertBbBrowserAvailable / openBbTab / closeBbTab / listBbTabs /
// evaluateBrowserJson now imported from @auto-job/browser/bb-shim.
// Backed by an in-process playwright connectOverCDP attachment to the
// dedicated Chrome at ~/.auto-job/chrome-profile (port 47320).
...
// runBb / runBbJson removed in the own-browser migration. Use bbFetch
// from @auto-job/browser/bb-shim for the one remaining `bb-browser fetch`
// call site (LinkedIn guest detail).
...
async function readLinkedInGuestJobDetail(url: string): Promise<ExternalAtsDetail | null> {
  ...
  try {
    const { stdout } = await bbFetch(guestUrl);
