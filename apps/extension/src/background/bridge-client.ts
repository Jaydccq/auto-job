...
export function bridgeClient(
  ...
  function pathFor(path: string, params?: Record<string, string | number>): string {
    if (!params) return path;
    return path.replace(/:([A-Za-z]+)/g, (_match, key: string) => {
      const value = params[key];
      if (value === undefined) {
        throw new Error(`missing bridge path parameter: ${key}`);
      }
  ...
  return {
    async getHealth() {
      ...
      try {
        const res = await fetch(base + pathFor(ENDPOINTS.HEALTH.path), {
          method: "GET",
          headers: headers(),
        });
    ...
    async checkLiveness(url: string) {
      ...
      return jsonRequest<LivenessResult>(
        pathFor(ENDPOINTS.LIVENESS.path),
        {
          method: "POST",
          headers: headers(),
    ...
    async createEvaluation(input: EvaluationInput) {
      ...
      return jsonRequest<EvaluateCreateResult>(
        pathFor(ENDPOINTS.EVALUATE_CREATE.path),
        {
          method: "POST",
          headers: headers(),
          body: JSON.stringify(env),
    ...
    async getJob(jobId: JobId) {
      try {
        const res = await fetch(base + pathFor(ENDPOINTS.JOB_GET.path, { id: jobId }), {
          method: "GET",
          headers: headers(),
        });
        const body = await res.json();
    ...
    async getTracker(limit: number) {
      ...
      return jsonRequest<TrackerListResult>(
        pathFor(ENDPOINTS.TRACKER_LIST.path),
        {
          method: "POST",
          headers: headers(),
          body: JSON.stringify(env),
    ...
    async getReport(num: number) {
      try {
        const res = await fetch(base + pathFor(ENDPOINTS.REPORT_READ.path, { num }), {
          method: "GET",
          headers: headers(),
        });
        const body = await res.json();
    ...
    async mergeTracker(dryRun: boolean) {
      ...
      return jsonRequest<MergeReport>(
        pathFor(ENDPOINTS.TRACKER_MERGE.path),
        {
          method: "POST",
          headers: headers(),
          body: JSON.stringify(env),
    ...
    async getAutofillProfile() {
      ...
      return jsonRequest<AutofillProfile>(
        pathFor(ENDPOINTS.AUTOFILL_PROFILE.path),
        {
          method: "POST",
          headers: headers(),
          body: JSON.stringify(env),
    ...
    async getAutofillResume() {
      ...
      return jsonRequest<AutofillResumeFile>(
        pathFor(ENDPOINTS.AUTOFILL_RESUME.path),
        {
          method: "POST",
          headers: headers(),
          body: JSON.stringify(env),
    ...
    async getNewGradPending(limit: number) {
      ...
      return jsonRequest<NewGradPendingResult>(
        pathFor(ENDPOINTS.NEWGRAD_PENDING.path),
        {
          method: "POST",
          headers: headers(),
          body: JSON.stringify(env),
    ...
    async backfillNewGradPendingCache(entries: NewGradPendingCacheBackfillInput[]) {
      ...
      return jsonRequest<NewGradPendingCacheBackfillResult>(
        pathFor(ENDPOINTS.NEWGRAD_PENDING_BACKFILL.path),
        {
          method: "POST",
          headers: headers(),
          body: JSON.stringify(env),
    ...
    async scoreNewGradRows(rows: NewGradRow[]) {
      ...
      return jsonRequest<NewGradScoreResult>(
        pathFor(ENDPOINTS.NEWGRAD_SCORE.path),
        {
          method: "POST",
          headers: headers(),
          body: JSON.stringify(env),
    ...
    async enrichNewGradRows(rows: EnrichedRow[]) {
      ...
      return jsonRequest<NewGradEnrichResult>(
        pathFor(ENDPOINTS.NEWGRAD_ENRICH.path),
        {
          method: "POST",
          headers: headers(),
          body: JSON.stringify(env),
    ...
    async streamEnrich(
      ...
      const res = await fetch(base + pathFor(ENDPOINTS.NEWGRAD_ENRICH_STREAM.path), {
        method: "POST",
    ...
    async streamJob(
      ...
      const res = await fetch(base + pathFor(ENDPOINTS.EVALUATE_STREAM.path, { id: jobId }), {
        method: "GET",
        headers: headers(),
        signal,
      });
