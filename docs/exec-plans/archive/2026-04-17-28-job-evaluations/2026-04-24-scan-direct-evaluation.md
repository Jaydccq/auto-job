...
`/auto-job scan` currently writes ordinary Greenhouse/Ashby/Lever API results
to `data/pipeline.md` and tells the user to run `/auto-job pipeline` later.
`/auto-job newgrad-scan` already performs the more useful flow: scan, enrich,
write survivors, queue `newgrad_quick` evaluations, wait for reports/tracker
rows, and record run artifacts.

## Goal

Let `/auto-job scan --evaluate` queue direct `newgrad_quick` evaluations for
...
- Direct evaluation should remain opt-in for `/auto-job scan` via
...
- 2026-04-24: User requested that pages extracted by `/auto-job scan` go
  through a flow like `/auto-job newgrad-scan`.
...
- Keep direct evaluation opt-in for generic `/auto-job scan` because this
...
Implemented and exercised. `/auto-job scan --evaluate` now supports the
